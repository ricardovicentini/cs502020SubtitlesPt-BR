...
function Player:checkRightCollision()
    if self.dx > 0 then
        if self.map:tileAt(self.x, self.y).id == FLAG_POLE_BOTTOM or
            self.map:tileAt(self.x, self.y).id == FLAG_POLE_MID or
            self.map:tileAt(self.x, self.y).id == FLAG_POLE_TOP 
         then
            -- reset y velocity
            self.dx = 0
            self.x = self.map:tileAt(self.x - 1, self.y).x * self.map.tileWidth
